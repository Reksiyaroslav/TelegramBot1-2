...
async def category_log_type_subject(callback: CallbackQuery):
    ...
    FmsContext.text = "Вы выбрали изменения темы урока"
    await  callback.message.answer("Оправте файл")
...
async def create_file(message: Message):
    ...
    if inform_text == "Вы выбрали изменения темы урока ":
    ...
    elif inform_text == "Вы выбрали средняя поешаемоть у преподователя":
