...
keyboard_main = InlineKeyboardMarkup(
    inline_keyboard=[
                     [InlineKeyboardButton(text="Средняя посещаемость у преподавателя", callback_data="average_rating_grop")]
                     ,[InlineKeyboardButton(text="Оценки студентов ", callback_data="student_assessment")],
                     [InlineKeyboardButton(text="Проверка дз у преподавателя", callback_data="homework_teacher")],
                    [InlineKeyboardButton(text="Проверка процента дз у студента", callback_data="homework_student")]])
...
keyboard_video = InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(text="Как связаться с разработчикам?",callback_data="what connect developer")]])